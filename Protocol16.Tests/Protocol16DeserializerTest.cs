...
public class Protocol16DeserializerTest
{
    ...
    public void DeserializeEventData()
    {
        ...
        Assert.That(result.Parameters[0], Is.EqualTo("test1"));
        Assert.That(result.Parameters[1], Is.EqualTo("test2"));
    }
    ...
    public void DeserializeOperationResponse()
    {
        ...
        Assert.That(result, Is.Not.Null);
        Assert.That(result.OperationCode, Is.EqualTo(100));
        Assert.That(result.ReturnCode, Is.EqualTo(101));
        Assert.That(result.Parameters[0], Is.EqualTo("test1"));
        Assert.That(result.Parameters[1], Is.EqualTo("test2"));
    }
    ...
    public void DeserializeOperationResponseWithDebugMessage()
    {
        ...
        Assert.That(result.OperationCode, Is.EqualTo(100));
        Assert.That(result.ReturnCode, Is.EqualTo(102));
        Assert.That(result.DebugMessage, Is.EqualTo("SomeDebug message"));
        Assert.That(result.Parameters[0], Is.EqualTo("test1"), "First parameter should be 'test1'.");
        Assert.That(result.Parameters[1], Is.EqualTo(2), "Second parameter should be 2.");
